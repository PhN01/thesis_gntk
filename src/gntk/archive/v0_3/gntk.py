...
1. solve aggregation step as matrix multiplication using the kronecker product
2. perform weighting of aggregated matrix (normalized) as matrix multiplication step
"""
...
def FC_layer(prev_sig_gxgx, prev_sig_gxgy, prev_sig_gygy, prev_theta, csig):
    """
    important insight (handwritten notes p.30/31): lambda can be calculated as a matrix
    hence: entire sigma update step can be done as a matrix operation
    TODO: solve sigma update as matrix operation
    """
...
def AGG(graph0, graph1, sig_gxgx, sig_gxgy, sig_gygy, theta, cu_type):
    ...
    if cu_type=="one":
        ...
        cu_mat = 1 / np.matmul(np.sum(graph0_adj, axis=1)[:, None], np.sum(graph1_adj, axis=0)[None, :])
        cu_mat_g0 = 1 / np.matmul(np.sum(graph0_adj, axis=1)[:,None], np.sum(graph0_adj, axis=0)[None,:])
        cu_mat_g1 = 1 / np.matmul(np.sum(graph1_adj, axis=1)[:,None], np.sum(graph1_adj, axis=0)[None,:])
