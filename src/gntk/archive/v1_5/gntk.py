...
implementation in gntk_v1_4.py.

Here I implement the following updates:
1. Option to return the GP matrix of the last layer of the neural network instead of the GNTK
"""
...
def gntk_pair(
    ...
    :param jk: GNTK parameter indicating if jumping knowledge is used in the graph
    neural network
    :param gp_mat: If true, return the GP Sigma matrix of the last step instead of the GNTK
    :return: returns a scalar kernel value for the similarity between g0 and g1
    """
    ...
    return theta if not gp_mat else np.array(sig).sum()
