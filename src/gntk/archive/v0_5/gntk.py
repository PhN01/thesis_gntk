...
1. Resolving redundancy of the following in each aggregation step
    kronecker product
    aggregation weight
    calculation of adjacency matrix
2. label matrices and adjacency matrices are provided to gntk function, e.g. as lists

Identified issues:
1. naming conventions partly inconsistent
2. entire sig_gxgx / sig_gygy stored, although only diagonal elements required for sigma updates
"""
...
def AGG(adjk, adjk_g0, adjk_g1, sig_gxgx, sig_gxgy, sig_gygy, theta, cu_mat, cu_mat_g0, cu_mat_g1):
...
def BLOCK(adjk, adjk_g0, adjk_g1, prev_sig_gxgx, prev_sig_gxgy, prev_sig_gygy, prev_theta, r, cu_mat, cu_mat_g0, cu_mat_g1, csig):
    ...
    sig_gxgx, sig_gxgy, sig_gygy, theta = AGG(adjk, adjk_g0, adjk_g1, sig_gxgx, sig_gxgy, sig_gygy, theta, cu_mat, cu_mat_g0, cu_mat_g1)
...
def gntk_pair(g0_lab, g1_lab, g0_adj, g1_adj, L, R, cu_type, csig):
    ...
    for block in range(1, L + 1):
        ...
        sig_gxgx, sig_gxgy, sig_gygy, theta = BLOCK(adjk, adjk_g0, adjk_g1, sig_gxgx, sig_gxgy, sig_gygy, theta, R, cu_mat, cu_mat_g0, cu_mat_g1, csig)
