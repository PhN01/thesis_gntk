...
def grid_search_cv(
    clf, train_indices, n_folds, param_grid, kernel_matrices,
):
    """
...
@ignore_warnings(category=ConvergenceWarning)
def train_and_test(train_indices, test_indices, matrices, n_classes, max_iterations):
    """
    ...
    if n_classes == 2:
        ...
        for i in range(n_scores):
            try:
                auroc = roc_auc_score(y_test_binarized[:, i], y_score[:, i],)
                auprc = average_precision_score(y_test_binarized[:, i], y_score[:, i])
...
if __name__ == "__main__":
    ...
    parser.add_argument("MATRIX", nargs="+", type=str, help="Input kernel matrix")
    parser.add_argument(
        "-f",
        "--force",
        action="store_true",
    ...
    parser.add_argument("-n", "--name", type=str, help="Data set name", required=True)
    parser.add_argument(
        ...
        for iteration in range(n_iterations):
            ...
            for fold_index, (train_index, test_index) in enumerate(
                ...
                results = train_and_test(
                    train_indices, test_indices, matrix, classes, args.max_iterations
                )
